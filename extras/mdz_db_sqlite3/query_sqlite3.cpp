...
bool Query_SQLite3::exec0(const ExecType &execType, bool recursion)
{
    ...
    for (const auto &inputVar : InputVars)
    {
        ...
        if (idx)
        {
            ...
            {
#if SQLITE_VERSION_NUMBER >= 3008007L
                sqlite3_bind_text64(stmt, idx, ABSTRACT_PTR_AS(VARCHAR, inputVar.second)->getValue(), ABSTRACT_PTR_AS(VARCHAR, inputVar.second)->getVarSize(), SQLITE_STATIC, SQLITE_UTF8);
#else
                // Only support 2GB data on older sqlite3 versions... (http://www.sqlite.org/releaselog/3_8_7.html) - WARNING: the compatibility should be enforced in source side, not using containers with >2gb capacity...
                // Also the encoding is not defined...
                sqlite3_bind_text(stmt, idx, ABSTRACT_PTR_AS(VARCHAR, inputVar.second)->getValue(), ABSTRACT_PTR_AS(VARCHAR, inputVar.second)->getVarSize(), SQLITE_STATIC);
#endif
            }
            break;
...
bool Query_SQLite3::step0()
{
    ...
    if (lastSQLReturnValue == SQLITE_ROW)
    {
        ...
        {
            bool bIsNull = sqlite3_column_type(stmt, columnpos) == SQLITE_NULL;
