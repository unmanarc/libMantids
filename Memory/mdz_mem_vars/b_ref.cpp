...
{
    if (!referencedBC) return std::make_pair(false,static_cast<uint64_t>(0)); // CANT APPEND TO NOTHING.
...
std::pair<bool,uint64_t> B_Ref::displace2(const uint64_t &roBytesToDisplace)
{
    ...
    if (!referencedBC) return std::make_pair(false,static_cast<uint64_t>(0));
